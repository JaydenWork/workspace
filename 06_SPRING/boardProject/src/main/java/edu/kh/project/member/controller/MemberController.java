...
@Controller //요청/응답 클래스 + bean 등록(Spring이 관

//@RequestMapping : 요청 주소에 맞는 클래스/메서드를 연결해준다
...
//	@RequestMapping(value="/login", method=RequestMethod.POST)
	public String login(HttpServletRequest req) {
		...
		// Spring Framework가 제공하는 Argument Resolver(매개 변수 해결사)가 해결해준다
...
//	@PostMapping("/login")
	public String login(/*@RequestParam("inputEmail")*/ String inputEmail,
						@RequestParam("inputPw")String pw) { //파라미터중에 inputEmail을 변수 email에 담겠다!
		
		///파라미터 전달 방법 2 : @RequestParam 어노테이션 사용(+생략 방법)
...
//		System.out.println(saveId);
		...
		// Model : 데이터 전달용 객체
		// -> 데이터를 K:V 형식으로 담아서(Map형식) 전달하는 방식
		// -> 기본적으로 request scope
		...
		if(loginMember !=null) {
			...
			path += referer; 						//"이전 페이지 주소"; // HTTP Header - referer(이전 주소)
			 ...
			 * 
			 * RedirectAttribute : 리다이렉트 시 데이터를 request scope로 전달할 수 있게하는 객체
			 * 
			 * 응답 전 : request scope
			 ...
			 * 
			 * */
			// addFlashAttribute : 잠시 Session에 추
