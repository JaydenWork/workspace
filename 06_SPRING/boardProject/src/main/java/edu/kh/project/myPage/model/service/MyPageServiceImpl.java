...
public class MyPageServiceImpl implements MyPageService{
	...
	public int updateProfile(MultipartFile profileImage, String webPath, String filePath, Member loginMember) throws IllegalStateException, IOException {
		...
		if(profileImage.getSize() > 0) {// 파일이 있다는 의미
			
			//1) 파일 이름 변경
			rename = Util.fileRename(profileImage.getOriginalFilename());
			
			// 2) 바뀐 이름 loginMember에 세팅
			loginMember.setProfileImage(webPath + rename); // /resources/images/member/20230510163910_12345.jpeg
		...
		// 프로필 이름 수정 DAO 메서드 호출
		int result = dao.updateProfileImage(loginMember);
		
		
		if(result > 0) { // 성공
			
			// 새 이미지가 업로드 된 경우(변경된 이미지가 있는 경우)
			if(rename != null) {
