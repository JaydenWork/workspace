...
const memberEmail = document.querySelector(
  "#loginFrm input[name ='memberEmail']"
);
...
btn1.addEventListener('click', () => {
  ...
  fetch('/selectMemberTel?nickname=' + inputNickname.value) // GET방식 요청 (파라미터를 쿼리스트링으로 추가)
    .then((resp) => resp.text())
    // resp : 응답 객체
    ...
    .then((tel) => {
      ...
      result1.innerText = tel; // 조회 결과를 result1 에 출력
    })
...
btn3.addEventListener('click', () => {
  fetch('/selectMemberList', {
    method: 'POST',
    headers: { 'Content-Type': 'application/text' }, // text를 보내겠다 / 문자열 하나를 파라미터로 전달
