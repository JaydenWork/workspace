...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		try {
			...
			DriverManager.getConnection( type + ip + port + dbName, user,pw);
			...
			// 3. SQL 작성
			// ** Java에서 작성되는 SQL은 마지막에 세미콜론을 작성하지 않는다**
		...
		} finally { // try에서 예외가 발생 하든 말든 수행
			// 4단계 : 사용한 JDBC 객체 자원 반환(close())
			// -> close 구문은 객체 생성 역순으로 작성
			
			// 생성 순서 : conn, stmt, rs
			// clost 순서 : rs, stmt, conn
