...
public class TestRun {
	 ...
	 public static void main(String[] args) {
		...
		System.out.println(c1.getMoney());  	// 부모것이지만 private이기 때문에 
		System.out.println(c1.getLastName());	//간접접근 방식으로 get/set을 통해 접근하면 사용할 수 있
...
//		System.out.println(c1.money());
		...
		//  -> private 필드/ 메서드는 상속이 되어 있읍.
		// 단, 자식도 접근만 못할뿐이다(private은 같은 클래스만 접근 가능)
		
		
		// 부모 클래스의 코드를 수정하면
		// 자식 모두에게 적용된다(공통적인규약, 유지보수성향상)
		
		System.out.println("---------------------------");
		// toString() : 객체가 가지고 있는 필드를 하나의 문자열로 만들어서 반환하는 것.
