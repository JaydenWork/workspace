...
//		=req.getRequestDispatcher("webapp폴더 기준으로 jsp 경로 작성");
		...
		// 2. request scope => 요청 받은페이지 + 위임 받은 페이지
		req.setAttribute("message", "request scope에 저장된 메시지 입니다");
		
		// 3. sessionScope -> 브라우저당 1개
		...
		session.setAttribute("str", "session 범위에세팅된 문자");
		
		application.setAttribute("str", "application 범위에세팅된 문자");
