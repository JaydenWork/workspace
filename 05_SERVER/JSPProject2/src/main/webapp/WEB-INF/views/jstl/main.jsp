...
<h1>JSTL(Jsp Standard Tag Library, JSP 표준 태그 라이브러리)</h1>
  ...
  <pre>
      JSP에서 자주 사용되거나 공통적으로 사용되는 java 코드(if, for, 변수선언, 형변환)

      스크립틀릿대신 html 태그 형식을 태그화하여 표준으로 제공하는 라이브러리
      (if, for 간단히 쓰고 싶은 이 라이브러리르 사용해라)

      <h3>jstl 라이브러리 다운로드 및 등록 방법 </h3>

      <ol>
          <li>
              <a href="https://tomcat.apache.org/download-taglibs.cgi">JSP다운로드 페이지 이동
              </a>
              <br>
          ...
          <li>
              ...
              taglib JSP 지시자 태를 추가 (2번줄에 작성된 내용)
          </li>
      </ol>
      ...
      <pre>
          -변수 선언(지정된 scope에 속성 세팅) 위한 태그명

          - c:set에 작성 가능한 속성
          1) var : 변수명(속성 key)
          2) value : 대입할 값
          3) scope : page, request, sesstion, application 중 하나 지정
                    (기본값 page)
...
<%-- requestScope에 세팅 --%>
<c:set var="num2" scope="request" value="30"/>
