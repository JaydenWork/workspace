...
public class CarService {
	...
	public void ex1() {
		...
		// 부모 참조 변수 = 자식 객체
		Car c2 = new Truck(); // 트럭 안에 있는 Car만 참조

		Car c3 = new LightCar(); // Lightcar에 있는 Car만 참조
		...
		// c2의 자료형이 Car
		// -> Truck 객체를 참조하더라도 Car 부분만 참조할 수 있음.

		// c2.setWeight(2.5); // 에러...c2로 접근 불가능하여 에러
...
//		== 다운 캐스팅
		...
		// 다운 캐스팅 + 얕은 복사(주소만 복사)
		LightCar light1 = (LightCar) c1; // LightCar로 강제 형변환
	...
	public void ex3() {
		...
		carList[1] = new LightCar(4, 4, "휘발유", 0.3); // 업캐스
		carList[2] = new Car(4, 2, "전기");

		// 향상된 for문(배열 요소에 대한 순차 접근)
		for (Car c : carList) {
			...
			if (c instanceof Truck) {
				...
				// ex) LightCar 객체를 Truck 참조변수가 참조하려고 할 때(상속이 적용되어야만 할 수 있음)

				// 해결방법 : instanceof를 통해 참조하는 객체의 자료형을 판단하여
				// 적절한 형식으로 다운 캐스팅 진행
			...
			System.out.println("차종이 등록되어 있지 않았습니다");
		}
...
//			printCar(Car객체 주소 또는 Car를 상속받은 객체 주소);
	...
	public void printCar(Car c) {
		...
		//edu.kh.polymorphism.ex1.dto.Car.toString()
