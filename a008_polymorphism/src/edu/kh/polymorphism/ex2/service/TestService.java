...
public class TestService {
	...
	public void ex1() {
		...
		// Animal a = new Animal();
		// Cannot instantiate the type Animal : 객체화 할 수 없는 타입니다
		// -> 추상 클래스이기 때문에!

		// 2. Animal을 상속 받은 자식 객체 생성
		Person p1 = new Person("포유류", "잡식", "정병종", 25);

		p1.breath();

		// 3.추상 클래스는 객체 생성x / but 부모 타입 참조 변수 가능
		// (다형성의 업캐스팅)
		Animal a1 = new Person("포유류", "채식", "김건강", 30);
		a1.breath(); // 동적 바인딩에 의해 Person에 구현된 breath()가 호출

		// 객체배열 + 추상 클래스
		Animal[] arr = new Animal[3];
		// Animal 객체 참조 변수의 묶음으로 생성한 것
		// (Animal 객체는 만들 수 없다)
		...
		System.out.println("-------------------------------");
		// 향상된 for문
		for (Animal a : arr) { // Animal 형태의 참조변수 a
			a.breath();
		}
	...
	public void ex2() {
		...
		// 2. 인터페이스로 객체 생성은 안되지만, 부모 타입 참조변수는 가능하다.
		//KH k = new KH; // 객체 생성 안됨.
		
		// 객체 배열 + 다형성 + 동적바인딩
		KH[] arr = new KH[2]; // KH 참조 변수의 묶음 생성
		
		arr[0] = new HongGilDong();
		arr[1] = new JungByungJong();
	...
	public void ex3() {
		...
		//Calculator cal = new KKWCalculator();
		Calculator cal = new NewCalculator();
	     ...
	     System.out.println("퀴즈에 참여하시겠습니까? (Y/N)");
