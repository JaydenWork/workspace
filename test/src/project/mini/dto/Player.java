...
public class Player implements Comparable<Player>{
	...
	public String getInfo(List<Stock> stocks) {
		...
		double rate = ((double)totalAssets - SEED_MONEY) / SEED_MONEY * 100;
	...
	public void sellStock(PlayerStock sellStock, int value, int price) {
		sellStock.setCount(sellStock.getCount()-value);
		cash += price*value;
		if(sellStock.getCount()==0) 
			for(int i=0;i<stocks.size();i++) 
				if(stocks.get(i).equals(sellStock)) 
					stocks.remove(i);
	}
	
	public void updateTotalAssets(List<Stock> stocks) {
		totalAssets = cash;
		for (PlayerStock ps : this.stocks) {
			for (Stock s : stocks)
				if (ps.getName().equals(s.getName())) {
					totalAssets += ps.count * s.getPrice();
					break;
				}
		}
	}
	...
	public int compareTo(Player comparePlayer) {
		return comparePlayer.totalAssets - this.totalAssets;
	}
}