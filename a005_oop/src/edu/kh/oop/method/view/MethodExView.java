...
public class MethodExView {
	...
	public void displayMenu() { // 메뉴 표시(출력) 기능
		...
		do {						      	  //  <---일단은 1번 실행한 후 조건을 검사하겠다!!!!!
			...
			case 2 : menu2(); break; //2입력 시 menu2() 메서드 호출
	...
	private void menu2() {
		...
		//service에서 제공하는 기능
		//threeNumbersSumAndAverage(정수1, 정수2, 정수3)를 호출 	
	...
	private void menu3() {
		...
		service.fiveRandomNumbers();
		
		int[] numbers = service.fiveRandomNumbers();
		// 1)service의 fiveRandomNumbers() 호출
		// 2)난수 5개가 저장된 int[](int형 배열) 이 반환됨(돌아옴)
		//    -> 반환된 int[]	은 실제 배열이 아닌
