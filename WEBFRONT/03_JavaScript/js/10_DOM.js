...
document.getElementById('btn1').addEventListener('click', () => {
  ...
  // 탐색 구문은 연달아서 작성 가능
  console.log(
    list[11].firstChild.nextSibling.nextSibling.nextSibling.nextSibling
  );
...
document.getElementById('btn2').addEventListener('click', () => {
  ...
  // 2 번째 요소 선택
  list[1].style.backgroundColor = 'violet';
  ...
  test.firstElementChild.nextElementSibling.addEventListener('click', () => {
    alert('첫 번째 요소의 다음 형제 요소입니다');
    test.firstElementChild.nextElementSibling.style.color = 'white';
  });
});

//innerHTML 을 이용해서 요소 추가하기
...
btn4.addEventListener('click', () => {
  ...
  if (count2 % 2 == 0) child.style.backgroundColor = 'pink';
});
