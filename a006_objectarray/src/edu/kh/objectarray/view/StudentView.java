...
public class StudentView {
	...
	private void selectIndex() {
		...
		//반환 받은 index번째 요소가 null이 아닐경우(학생 정보 있을 경우)에만 출력을 하겠다.
		// -> NullPointerException 방지 처리
		if(std != null) {
			System.out.println(std.toString());
		}else {
			System.out.println("해당 인덱스 번째 학생 정보가  존재하지 않습니다...");
		}
	...
	private void selectName() {
		...
		if(resultArr != null) {
			...
			for(Student s : resultArr) {
				...
				if(s == null) break;
				// if / else / else if / else / for / while 등
