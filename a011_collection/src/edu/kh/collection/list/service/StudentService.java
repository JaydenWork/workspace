...
public class StudentService {
	...
	public StudentService() {
		...
		studentList.add(new Student("정병종", 3, 5, 17, "서울시 중구 남대문로", 'M', 75));
		studentList.add(new Student("정민영", 2, 7, 7, "서울시 성동구 아차산로", 'F', 85));
		studentList.add(new Student("최대윤", 2, 1, 11, "서울시 도봉구 도봉로", 'F', 65));
		studentList.add(new Student("김재창", 3, 9, 25, "서울시 강남구 청담동", 'M', 95));
		studentList.add(new Student("이준규", 1, 3, 15, "제주도 서귀포시 청계로", 'M', 90));
	...
	public boolean addstudent(Student std) {
		return studentList.add(std); // add 매서드가 boolean트루를 반환한다(무조건)
	}
	 ...
	 * 
	 * @param name
	 * @return list:List<Student> 이름이 일치하는 학생리스트
	 */
	public List<Student> selectName(String name) {
		...
		for (Student s : studentList) { // s에 Student한명한명이 담긴다
	...
	public List<Student> selectAddress(String input) {
		...
		for (Student s : studentList) { // s에 Student한명한명이 담긴다
			...
			// String.comtains("문자열) : String에 "문자열"이 포함되어 있으면 true 반환
