...
public class SwitchEx {
	 ...
	 * -> 식의 결과에 맞는 case문이 수행된다
	 * 
	 * 
	 * [작성법]
	 * switch(식){ // 식 수행 결과는 true/false가 아닌값(정수, 문자열 제일 많이씀)
	 * 
...
//	}
		...
		public void ex3() {
			...
			default : price = -1; // 잘 못 입력한 경우에 사용하지 않는 숫자 (-1)을 이용해서 표현
			
			}
			
		
			if(price != -1) {
				System.out.printf("%s은 %d원 입니다.", input, price);
			} else { // price가 -1인 경우
		...
		public void ex4(){
			...
			case "/":
				if(num1 == 0 || num2 == 0) {
					...
					System.out.printf("%d %s %d = %.3f", num1, op, num2, (double)num1/num2);break;
				}
