...
//	            case 3: deleteComment(boardNo); break; // 댓글 삭제
			...
			// 2-2) "정말로 삭제하시겠습니까? (Y/N) : " 출력 후
			// Y입력 시 : 삭제 서비스 호출(댓글번호)
			// N입력 시 : "최소되었습니다";
	...
	private void insertComment(int boardNo) {
		...
		try {
			// 댓 삽입 서비스 호출
			int result = service.insertComment(commentContent, Session.loginMember.getMemberNo());
			if(result > 0) { // 성공
				System.out.println("\n=== 등록 되었습니다 ===\n");
				...
				System.out.println("--------------------------------------------------------");
				System.out.printf("댓글 번호: %d \n댓글 내용 : %s\n 작성 회원 번호 : %d\\n", comment.getCommentContent(),comment.getCommentNo());
				System.out.println("--------------------------------------------------------\n");
